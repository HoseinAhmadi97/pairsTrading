...
class prediction_models():
    """
    A class For prediction and Classification on NN Models
    """
    def __init__(self, data, inital_train_len, iteration_num) -> None:
        self.data = data
        self.inital_train_len = inital_train_len
        self.iteration_num = iteration_num
    
    def NN_prediction(self):
        prediction = np.zeros(len(self.data) - self.inital_train_len)
        for i, leng in enumerate(range(self.inital_train_len, len(self.data))):
            data_ = self.data[:leng]
            X = data_.drop(columns = ['f_return_1', 'label'])
            y = data_['f_return_1']
            ...
            #model
            all_predictions = np.zeros(self.iteration_num)
            for j in range(self.iteration_num):
                nernum = 5
                nn_model = MLPRegressor(hidden_layer_sizes=(nernum, nernum,), solver = 'lbfgs', activation='tanh', max_iter=10000)
                fitted_nn_model = nn_model.fit(X_train, y_train)
                all_predictions[j] = fitted_nn_model.predict(X_test)[0]
                
            #one step prediction
            prediction[i] = np.mean(all_predictions)
    ...
    def NN_Classifier(self):
        prediction = np.zeros(len(self.data) - self.inital_train_len)
        X = self.data.drop(columns = ['f_return_1', 'label'])
        y = self.data['label']

        X_train = X[:self.inital_train_len]
        X_test = X[self.inital_train_len: ]
        y_train = y[:self.inital_train_len]
        y_test = y[self.inital_train_len: ]

        nernum = 5
        all_predictions = np.zeros((len(prediction), self.iteration_num))
        for j in range(self.iteration_num):
            cl_model = MLPClassifier(hidden_layer_sizes=(nernum, nernum,), solver = 'lbfgs', activation='tanh', max_iter=10000)
            fitted_cl_model = cl_model.fit(X_train, y_train)
            all_predictions[:, j] = fitted_cl_model.predict(X_test)
            
        prediction = all_predictions.mean(axis=1)
